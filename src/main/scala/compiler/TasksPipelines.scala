...
object TasksPipelines {
  ...
  def verifier(outputDirPath: Path, timeoutSecOpt: Option[Int], logger: String => Unit): CompilerStep[List[SourceCodeProvider], Boolean] = {
    val er = createErrorReporter
    MultiStep(frontend(er))
      .andThen(new ContextCreator(er, FunctionsToInject.functionsToInject))
      .andThen(new TypeChecker(er))
      ...
      .andThen(new PathsVerifier(Z3Solver(outputDirPath), timeoutSecOpt, er, logger))
  }
